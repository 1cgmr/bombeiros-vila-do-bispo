...
`institutionalPage` não é uma coleção de páginas livres. O Studio expõe seis documentos fixos:

| ID estável | Navegação editorial |
| --- | --- |
| `institutional-page-association` | Associação → Apresentação |
| `institutional-page-statutes` | Associação → Estatutos |
| `institutional-page-social-bodies` | Associação → Órgãos Sociais → apresentação |
...
Campos: título, introdução, imagem acessível, Portable Text controlado, referências ordenadas a documentos institucionais e SEO. A apresentação da Associação tem ainda o campo exclusivo «Mensagem do Presidente», que pode ficar vazio até existir texto aprovado. A página Estatutos tem uma lista editável de exemplos provisórios de artigos, apresentada enquanto não houver PDF oficial associado. Os PDFs oficiais são associados através de referências a `institutionalDocument` com a categoria `statutes`. A composição dos órgãos sociais é editada nos documentos `governingBody` com o tipo `governingBody`, organizados no menu Associação. Estes documentos usam as mesmas proteções de ações dos singletons.

O script `studio/scripts/add-association-sections.ts` cria, apenas quando faltam, apresentações provisórias para Estatutos e Órgãos Sociais, três secções ilustrativas e quatro artigos de exemplo dos Estatutos, dois órgãos sociais de exemplo e um documento de Estatutos em rascunho, sem PDF. Só acrescenta as secções à página publicada de Estatutos se esta conservar exatamente o parágrafo provisório inicial e não tiver rascunho. Os artigos são acrescentados apenas quando o campo ainda não existe e a página não tem rascunho; uma lista editada ou esvaziada pelo editor é preservada. Executa-se em `studio/` com `npm run content:association:dry-run` e `npm run content:association:apply`. O documento de exemplo só pode ser publicado depois de ser substituído pelo título, resumo e PDF oficiais. A mensagem do Presidente continua vazia até existir texto aprovado.
...
- Os seis conteúdos institucionais usam um único schema controlado com IDs fixos, evitando páginas arbitrárias e schemas quase idênticos.
